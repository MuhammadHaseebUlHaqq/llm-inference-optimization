...
- OSS target: SGLang, but the original reasoning here was wrong and the correction matters. The theory was that a lower contributor count than vLLM means higher visibility per PR. Contributor count is not the constraint. Review throughput is. Measured on 2026-09-08: SGLang carried 4,318 open pull requests, 2,446 of them older than one month. A sample of 12 recent merges was 6 maintainers and 6 prior contributors, with no first-time authors, and every outside merge was vendor hardware enablement (AMD gfx950, Intel XPU, NPU). Writing a good PR is not the hard part; getting anyone to look at it is.
- What this means in practice. Do not plan around "submit a PR and it gets merged." The paths that actually work are (a) vendor hardware enablement, which is staffed and has real maintainer relationships, and (b) a smaller project with genuine review capacity. Both are long games. A measured, published artifact does not depend on anyone upstream and should be treated as the primary deliverable, with a merge as upside. See `docs/sglang-contribution-runbook.md` and `blog/sglang-cuda-graph-cliff.md` for the worked example: a real bug, reproduced on rented hardware for under $2, published in full, with the upstream PR correctly not attempted because it already existed as #33900.
- Before planning any contribution, check for an existing PR, not just an unassigned issue. GitHub's linked-PR field is often empty even when a PR exists. Search the repo for PRs referencing the issue number. Of seven promising unassigned SGLang issues checked, five already had an open PR.
